...
[TestFixture]
public partial class CardFoundationAdminPages : PageTest
{
    [Test]
    public async Task MainNavigation()
    {
        ...
        await Page.GetByTestId("cardFoundationNavLink").ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Foundation Home");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Create New" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Foundation Add");

        await Page.GetByLabel("Ordering:", new() { Exact = true }).FillAsync("1");
        // CreatePropertyCodePlaceholder

        await Page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Foundation View");
