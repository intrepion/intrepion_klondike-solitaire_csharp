...
[TestFixture]
public partial class CardStockAdminPages : PageTest
{
    [Test]
    public async Task MainNavigation()
    {
        ...
        await Page.GetByTestId("cardStockNavLink").ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Stock Home");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Create New" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Stock Add");

        await Page.GetByLabel("Ordering:", new() { Exact = true }).FillAsync("1");
        // CreatePropertyCodePlaceholder

        await Page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Stock View");
