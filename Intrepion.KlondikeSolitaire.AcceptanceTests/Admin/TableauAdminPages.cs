...
[TestFixture]
public partial class TableauAdminPages : PageTest
{
    [Test]
    public async Task MainNavigation()
    {
        ...
        await Page.GetByTestId("tableauNavLink").ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Tableau Home");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Create New" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Tableau Add");

        await Page.GetByLabel("Pile Index:", new() { Exact = true }).FillAsync("1");
        // CreatePropertyCodePlaceholder

        await Page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Tableau View");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Edit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Tableau Edit");
