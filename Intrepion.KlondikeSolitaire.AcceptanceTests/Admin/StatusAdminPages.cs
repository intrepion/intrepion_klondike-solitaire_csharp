...
[TestFixture]
public partial class StatusAdminPages : PageTest
{
    [Test]
    public async Task MainNavigation()
    {
        ...
        await Page.GetByTestId("statusNavLink").ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Status Home");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Create New" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Status Add");

        await Page.GetByLabel("Name:", new() { Exact = true }).FillAsync("aName" + aRandomString);
        // CreatePropertyCodePlaceholder

        await Page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Status View");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Edit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Status Edit");
