...
[TestFixture]
public partial class CardWasteAdminPages : PageTest
{
    [Test]
    public async Task MainNavigation()
    {
        ...
        await Page.GetByTestId("cardWasteNavLink").ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Waste Home");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Create New" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Waste Add");

        await Page.GetByLabel("Ordering:", new() { Exact = true }).FillAsync("1");
        // CreatePropertyCodePlaceholder

        await Page.GetByRole(AriaRole.Button, new() { Name = "Submit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Waste View");
        await Page.GetByRole(AriaRole.Link, new() { Name = "Edit" }).ClickAsync();
        await Expect(Page).ToHaveTitleAsync("Card Waste Edit");
